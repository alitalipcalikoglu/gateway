...
None. `routes.json` (path from `ROUTES_FILE`, default `./routes.json`) is read once at startup
(`RouteTable.load`, `src/application.js`) and validated; a bad file stops the process with the
exact problem (`ConfigError`). No database, no files written at runtime.
...
call is treated as "unavailable" per the fail-open/closed rule below, not retried).
...
All process-local, reset on restart — there is nowhere else for a stateless service to keep them.

## Logging

Custom access-log line (not Fastify's default; `disableRequestLogging: true`) with `route`,
`method`, `path`, `status`, `durationMs`, `ip`, `upstream`, `ua`, `reqId`, and — as of this stage —
`traceId`. See [OBSERVABILITY.md](../../stack/docs/OBSERVABILITY.md) for the full target
vocabulary and what is still missing (`service`, `version`, `spanId`, `upstreamMs`).
...
is up to N× the configured value), the passive upstream-cooldown state in `UpstreamPool` (one
instance marking an upstream down does not inform the others), and the geo lookup cache
...
- An upstream that is down: served 502/504 for that request; after one failure it is marked down
  for `UPSTREAM_COOLDOWN_MS` and skipped by that instance's pool (not other instances').
- JWKS unreachable: every `auth: "user"` request gets `503 AUTH_UNAVAILABLE` until it recovers.
- ratelimit/geo unreachable: per-route `policy.failOpen` (default `true`) decides — `true` lets the
  request through and increments `gateway_dependency_errors_total`; `false` answers
  `503 RATE_LIMIT_UNAVAILABLE`. `geo: true` routes always fail open (empty `X-Geo-*` headers), there
  is no fail-closed option for geo.
...
- Misconfigured route referencing an `injectApiKey` env var that is unset, or a `policy`/`geo`
  route with the corresponding service not configured: caught at startup (`ConfigError`), the
  process refuses to start rather than serving broken routes.
